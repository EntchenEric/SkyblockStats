...
function getWispBulkwarkStats(kills: number) {
  ...
  for (const [key, value] of Object.entries(BulkwarkStats)) {
    if (kills < 100) { return [parseInt(key), kills, 0, 0] }
    if (kills >= parseInt(key)) {
      if (kills >= 200000) {
        return ['§a§lMAXED OUT! NICE!', kills, value.Defense, value.TrueDefense];
      }
      return [parseInt(key), kills, value.Defense, value.TrueDefense];
    }
    lastkey = key;
  }
}
  ...
  if (id.includes('WISP')) {
    for (let i = 0; i < profileData.length; i++) {
      if (profileData[i].type.includes('WISP') && profileData[i].uuid === uuid) {
        ...
        if (blazekills == undefined) blazekills = 0;
      }
    }
  };
